  ...
  return (
    <>
      ...
      {alarmEvents.length > 0 ? (
        <div className="mt-6 rounded-2xl border border-red-500/20 bg-red-500/10 p-4 backdrop-blur-md">
          <div className="text-sm font-semibold text-black dark:text-white">Active alarms</div>
          <div className="mt-3 space-y-2">
            {alarmEvents.slice(0, 8).map((a) => (
              <div
                key={`${a.tankId}-${a.timeIso}-${a.reason}`}
                className="rounded-xl border border-red-500/10 dark:border-red-500/20 bg-white/50 dark:bg-black/20 px-3 py-2 text-sm text-black/85 dark:text-white/85 transition-colors"
              >
                <span className="font-medium text-black dark:text-white">{a.tankName}</span>
                <span className="mx-2 text-black/30 dark:text-white/30">•</span>
                <span className="text-red-600 dark:text-red-400">{a.reason}</span>
              </div>
            ))}
          </div>
        </div>
